...
async def reports_service_health():
    ...
    return {
        ...
        "status": "operational",
        "endpoints": ["/generate", "/template", "/my-reports"]
    }
...
async def get_my_reports(
    ...
    """Return reports generated by the authenticated user."""
    try:
        db = await get_db()
        if db is None:
        ...
        query = (
            db.table("settle_reports")
            .select("*")
            .eq("api_key_id", str(api_key_data.get("api_key_id", "")))
            .order("created_at", desc=True)
            .limit(limit)
        )
        result = query.execute()
        reports = result.data if result and result.data else []
